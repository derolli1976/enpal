...
## Branch Context
Current branch: 3.1.1 release prep (manifest version `3.1.1`).
3.1.1 adds a repair issue for entries fixed to HTML mode on firmware 8.51+: `html_client.py` parses `firmware_version` on every fetch, `sensor.py` `_manage_html_mode_issue()` raises a fixable ERROR issue (`html_mode_unsupported_firmware_<entry_id>`, threshold `HTML_MODE_BROKEN_FIRMWARE = (8, 51)` in `const.py`), and `repairs.py` `HtmlModeUnsupportedRepairFlow` switches `data_source` to `websocket` after one confirmation. Release notes: [docs/RELEASE_NOTES_3.1.1.md](../docs/RELEASE_NOTES_3.1.1.md).
