...
**Data-source modes** (chosen in the config flow, stored as `data_source` = `html` / `websocket` / `influxdb` / `auto`):
...
- **InfluxDB mode (Expert, 3.2.0+)**: Polls the box's local InfluxDB 2.x (port 8086, bucket `solar`, org `enpal`; token from Enpal support via the app chatbot). `api/influx_client.py` runs one Flux `last()` query per poll and maps the rows through the same alias/group/unit pipeline, producing identical entity ids. Numeric values only (no wallbox status/mode strings, no serials); wallbox control/status stays on the Blazor client. Never chosen by auto-detect.

**Firmware dependency**: The WebSocket mode, native wallbox control, and the incremental RenderBatch parsing require Enpal firmware **Solar Rel. 8.50** or later (tested with `8.50.1` and `8.51.0` on Huawei and FoxESS systems). On 8.51 the HTTP page only pre-renders "Site Data"; all device rows arrive via WebSocket RenderBatches, and some rows are hidden behind per-card checkboxes the client enables itself. Older firmware keeps working in HTML mode only.
...
## Release Process
These steps are identical for every release (beta and final):

1. **Manifest**: Set the version in `custom_components/enpal_webparser/manifest.json` (beta: `X.Y.Zb<n>`, final: `X.Y.Z`).
2. **Release Notes**: Create `docs/RELEASE_NOTES_<version>.md`. For a final release, consolidate the beta release notes into one document and remove beta-specific hints. Style: German, "du"-address, see "Documentation Writing Style" below. Standard sections: Was ist neu / Fixes, Getestet, Kompatibilität, Unterstützung (sponsoring block).
3. **README**: Rewrite the "🆕 Neu in Version X" section for the new version and link the new release notes. For a final release, merge the beta sections. Older versions remain as "Aus Version ..." subsections. Check the firmware badge (`tested with enpal firmware`) and update if needed.
4. **Copilot instructions**: Update the "Branch Context" section in this file (current version, short summary of the changes, link to the release notes).
5. **Tests**: Run `pytest` with `PYTHONPATH` set to the project root. All tests must pass.
6. **Pull Request**: PR from `dev` to `main` with the template from `.github/pull_request_template.md` filled in.
7. **GitHub Release**: After the merge, create a release with tag `v<version>` on `main`. Use the release notes content as the description. Mark beta releases as "Pre-release". HACS reads the version from the manifest.

## Branch Context
Current branch: 3.2.0 release prep (manifest version `3.2.0`, final release consolidating the 3.2.0b1/b2 betas; combined release notes: [docs/RELEASE_NOTES_3.2.0.md](../docs/RELEASE_NOTES_3.2.0.md)).
3.2.0b2 fixes issue #180 (Inverter system state unavailable on Sungrow, firmware 8.51): Sungrow ships the state bitfield under `Inverter.Running.State` (with vendor-specific bit labels like "PV power generated"), while `Inverter.System.State` is a short plain value ("Running (64)"); Huawei is the other way around. The WS fast path now detects the bitfield by VALUE (`_is_system_state_value`: "Bits" in value plus length/regex check after tag strip) instead of hardcoding the key; `_apply_system_state_row` derives key/group/base-sensor name from the row. Bit labels are parsed from the blob itself (`parse_inverter_state_bits`, `Bit N: <label>: ✓/✗`, literal check marks in the WS payload); `_HISTORIC_BIT_LABELS` normalizes the two long Huawei blob labels (bits 3/4) to the historic short forms so existing Huawei entity ids stay stable; `INV_BITS` remains only as fallback for label-less blobs (InfluxDB synthesis). Sungrow gets correct new split entities (`inverter_system_state_pv_power_generated` etc., with ICON_MAP entries); old mislabeled entities on Sungrow boxes become orphans by design. Release notes: [docs/RELEASE_NOTES_3.2.0b2.md](../docs/RELEASE_NOTES_3.2.0b2.md).
3.2.0b1 adds the InfluxDB expert data source (see above): `api/influx_client.py` (`EnpalInfluxClient`, `parse_influx_csv` tolerant of blank lines/repeated headers), config flow option "influxdb" in both the initial configure step and the options flow (token/org/bucket fields appear only after the source is selected; `validate_influx_settings` test-connects on save, no silent fallback), `_manage_wallbox_status_issue` suppressed in influx mode, wallbox client native for websocket AND influxdb. Fixture `tests/fixtures/influx_last_values.csv`. Schema facts (verified live, InfluxDB 2.8.0): 6 measurements = card groups, field keys = the deviceMessages dotted keys, `unit` tag (`Percent`/`Celcius`[sic]/W/V/A/Wh/kWh/Hz/None), junk field `measureId`, `Inverter.System.State` decimal-only (bits are synthesized). Release notes: [docs/RELEASE_NOTES_3.2.0b1.md](../docs/RELEASE_NOTES_3.2.0b1.md).
Previous release 3.1.2 fixed three firmware-8.51.1 issues: `/wallbox` no longer pre-rendered + status card renamed "Status" → "Connector" (`wallbox_client._extract_status_text` accepts both labels, `get_wallbox_data` keeps the WS connection fresh), issue #177 (DC power source selection moved into `CumulativeEnergySensor`, retried per update, warns once; no forced Huawei fallback), issue #178 (WS path feeds the base `inverter_system_state` sensor again, tag-free and truncated to 240 chars).
Previous release 3.1.0 (issue #148 firmware 8.51 support) consolidates the 3.0.3b1–b13 beta series that restored full functionality on firmware **Solar Rel. 8.51**. Key changes: (1) On 8.51 the HTTP page only pre-renders "Site Data"; the device tables arrive as a big initial RenderBatch over the WebSocket. `render_batch.py` gained `extract_initial_rows()` which detects rows by the dotted-key pattern (`_KEY_RE`, uppercase first segment) instead of `dp-flash` markers or key whitelists; `websocket_client._extract_rows()` merges initial and dp-flash rows (dp-flash wins). (2) Hidden rows ("Show unsupported/internal values" checkboxes) are enabled by the client via `DispatchEventAsync`; the historic click failures (b7–b10) were caused solely by an extra `"type": "change"` field in the event args — `ChangeEventArgs` rejects unknown members; correct payload is `{"value": true}` (`MouseEventArgs` for wallbox buttons HAS a Type property, which is why those always worked). (3) Group selection is now soft: `parse_enpal_html_sensors(html, groups, excluded_groups)` parses every card; deselected groups only set `enabled=False` (registry default). `excluded_groups_from_options()` in `utils.py` derives exclusions; entries without the stored option fall back to `LEGACY_GROUP_CHOICES` so groups added later (e.g. `ControlBox`) never count as deselected. (4) Unknown WS keys become "Uncategorized" sensors (junk guard: key must start with a letter). (5) `Inverter.System.State` arrives as an >800-char HTML `<ul>` blob on 8.51; `_apply_system_state_row()` strips tags and reuses `expand_inverter_system_state`. (6) `SENSOR_KEY_GROUPS` grew by ~115 keys (FoxESS/ControlBox capture + evidenced 8.50 Huawei keys with their historic groups). Validated via replays of recorded WS streams (fixtures in `tests/fixtures/render_batch_851_initial.bin`) and live tests against Huawei and FoxESS boxes. Release notes: [docs/RELEASE_NOTES_3.1.0.md](../docs/RELEASE_NOTES_3.1.0.md).
Analysis tooling from this work lives in `scripts/`: `phase2_snapshot_box.py`, `phase2_browser_capture.py` (Playwright real-browser WS capture), `phase2_click_experiment.py`, `sniff_websocket.py`.
