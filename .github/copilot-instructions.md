...
## Branch Context
Current branch: 3.0.1 release prep (manifest version `3.0.1`).
Recent work: 3.0.1 is a bugfix release for the WebSocket mode. Three fixes resolve a sensor that could be `unavailable` or frozen after a Home Assistant restart (mainly `Energy.Consumption.Total.Lifetime`): (1) the push handler in `sensor.py` no longer calls `async_set_updated_data`, so a WebSocket push does not reset the coordinator's poll timer and the 60 s full scrape keeps running (and re-adds sensors that were missing at startup); (2) a numeric guard in `websocket_client._apply_diff` (`_NUMERIC_DEVICE_CLASSES` + `is_strict_number`) prevents a timestamp-only RenderBatch row from overwriting a numeric sensor with a non-parseable value; (3) the wallbox startup warnings in `switch.py`/`select.py` were downgraded to `debug` (startup ordering race, harmless). Release notes live in [docs/RELEASE_NOTES_3.0.1.md](../docs/RELEASE_NOTES_3.0.1.md). The 3.0.0 highlights (Firmware-8.50 WebSocket/native wallbox mode, dynamic runtime sensor creation with `RestoreEntity`, incremental RenderBatch parsing, second wallbox status candidate `wallbox_status_connector_1`, repair flow in `repairs.py`) remain in [docs/RELEASE_NOTES_3.0.0.md](../docs/RELEASE_NOTES_3.0.0.md).
